...
@app.route('/skill_icon/<int:skill_id>')
def serve_skill_icon(skill_id):
    conn = get_db_connection()
    # Ensure you are selecting icon_mimetype here!
    skill = conn.execute('SELECT icon, icon_mimetype FROM skills WHERE id = ?', (skill_id,)).fetchone()
    conn.close()

    if skill and skill['icon']:
        return send_file(
            io.BytesIO(skill['icon']),
            mimetype=skill['icon_mimetype'] or 'image/png' # Fallback to png if empty
        )
    # Fallback to checkmark if no custom icon exists
    return redirect(url_for('serve_media', category='icon', identifier='checkmark'))
...
def edit_skill(id):
    ...
    if request.method == 'POST':
        ...
        if icon_file and icon_file.filename != '':
            ...
            icon_mimetype = icon_file.content_type
            conn.execute('UPDATE skills SET name=?, level=?, category=?, icon=?, icon_mimetype=? WHERE id=?',
                         (name, level, category, icon_binary, icon_mimetype, id))
        else:
            # No new file uploaded, keep the existing icon
            conn.execute('UPDATE skills SET name=?, level=?, category=? WHERE id=?', 
...
def add_skill():
    ...
    if request.method == 'POST':
        ...
        # 1. Capture the file from the 'icon_file' input field
        icon_file = request.files.get('icon_file')
        
        # 2. Convert the image to binary data if a file was uploaded
        icon_binary = icon_file.read() if icon_file and icon_file.filename != '' else None
        icon_mimetype = icon_file.content_type
        conn = get_db_connection()
        # 3. Insert the binary data (icon_binary) into the database
        conn.execute('INSERT INTO skills (name, level, category, icon, icon_mimetype) VALUES (?, ?, ?, ?, ?)',
                     (name, level, category, icon_binary, icon_mimetype))
        conn.commit()
        conn.close()
        return redirect(url_for('index'))
